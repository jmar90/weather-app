...
const url = 'https://api.darksky.net/forecast/' + DARKSKY_KEY + '/37.826,-122.4233?units=si';
...
request({ url: url, json: true }, (error, response) => {
	if(error){ //If low-level OS error (eg, no internet or API down)
		console.log('Unable to connect to weather service.');
	} else if(response.body.error){
		console.log('Unable to find lcoation');
	} else {
...
request({ url: geoCodeURL, json: true }, (error, response) => {
	...
	} else if(response.body.features.length === 0){ //If there is an error, the length of features array will be 0
